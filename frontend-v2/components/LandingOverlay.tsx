...
export default function LandingOverlay({ onEnter }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      i++;
      if (i > fullText.length) { clearInterval(timer); setShowButton(true); }
    }, 25);
      ...
      <div style={{
        ...
        <div style={{ transition: 'opacity 0.8s', opacity: showButton ? 1 : 0 }}>
          ...
          <p style={{ color: '#6b7280', fontSize: '11px', lineHeight: 1.5, marginBottom: '24px', maxWidth: '360px' }}>
            Play skill games, wager G$, compete on weekly leaderboards. Every wager funds GoodDollar UBI — play to earn, play for good.
          </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            <button onClick={() => { onEnter(); router.push('/arena'); }} style={{
              ...
              cursor: 'pointer', transition: 'all 0.2s',
            }}>CHALLENGE AI AGENT</button>

            <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', marginTop: '8px' }}>
              {[
                { label: 'GET G$', href: 'https://gooddollar.org', internal: false },
                { label: 'TELEGRAM', href: 'https://t.me/+oY4inbBoglViNmE0', internal: false },
                ...
                <a key={i}
                  ...
                  onClick={link.internal ? (e) => { e.preventDefault(); onEnter(); router.push(link.href); } : undefined}
                  style={{ color: '#4b5563', fontSize: '9px', fontWeight: 700, letterSpacing: '1px', textDecoration: 'none' }}
                >{link.label}</a>
              ))}
            </div>
          </div>
